...
export function registerValidation(
  ...
  const validateOpen = (doc: vscode.TextDocument): void => {
    if (!isManifest(doc)) { return; }
    collection.set(doc.uri, validate(doc));
  };

  for (const doc of vscode.workspace.textDocuments) { validateOpen(doc); }
...
function isManifest(doc: vscode.TextDocument): boolean {
  return doc.fileName.endsWith("shipwright.json")
    || /fixtures[\\/]manifests[\\/].*\.json$/.test(doc.fileName);
}
...
function validateProduct(
  doc: vscode.TextDocument, text: string,
  manifest: ShipwrightManifest, diags: vscode.Diagnostic[],
): void {
  ...
  if (!manifest.product.version) {
    diags.push(makeDiag(doc, findOffset(text, '"product"'), "Product version is required", vscode.DiagnosticSeverity.Error));
  }
}
...
function validateSingleComponent(
  doc: vscode.TextDocument, _text: string,
  comp: Component, offset: number, diags: vscode.Diagnostic[],
): void {
  const isExecutable = ["cli", "lsp", "mcp", "sidecar", "dap", "tool"].includes(comp.kind);
  if (isExecutable && !comp.binaryName) {
    diags.push(makeDiag(doc, offset, `Component "${comp.id}" (${comp.kind}) requires binaryName`, vscode.DiagnosticSeverity.Error));
  }
  if (isExecutable && !comp.expectedVersion) {
    diags.push(makeDiag(doc, offset, `Component "${comp.id}" (${comp.kind}) requires expectedVersion`, vscode.DiagnosticSeverity.Error));
  }
  if (isExecutable && (!comp.sources || comp.sources.length === 0)) {
    diags.push(makeDiag(doc, offset, `Component "${comp.id}" (${comp.kind}) requires at least one source`, vscode.DiagnosticSeverity.Error));
  }
  if (comp.kind === "asset" && !comp.asset) {
    diags.push(makeDiag(doc, offset, `Component "${comp.id}" (asset) requires an asset block`, vscode.DiagnosticSeverity.Error));
  }
  if (comp.platforms) {
    for (const p of comp.platforms) {
      if (!ALL_PLATFORMS.includes(p)) {
        diags.push(makeDiag(doc, offset, `Unknown platform "${p}" in component "${comp.id}"`, vscode.DiagnosticSeverity.Warning));
      }
    }
  }
  if (comp.bundled && comp.sources && !comp.sources.includes("bundled")) {
    diags.push(makeDiag(doc, offset, `Component "${comp.id}" has bundled config but "bundled" is missing from sources`, vscode.DiagnosticSeverity.Warning));
  }
}
