...
import {
  ...
  deleteTempFile, waitForDiagnostics, diagMessages,
} from "./helpers";
...
suite("Custom Editor Provider", () => {
  ...
  test("opens manifest in custom editor, syncs edits back to text document", async () => {
    const content = JSON.stringify({
      ...
      components: [{
        id: "ed-lsp", kind: "lsp", language: "rust", binaryName: "ed-lsp",
    ...
    try {
      ...
      await vscode.commands.executeCommand(
        "vscode.openWith", uri, "shipwright.manifestEditor",
      );
      ...
      const newContent = JSON.stringify({
        ...
        components: [{
          id: "ed-lsp", kind: "lsp", language: "rust", binaryName: "ed-lsp",
          expectedVersion: "2.0.0", platforms: ["linux-x64"], sources: ["bundled"],
        }],
      }, null, 2);
      ...
      assert.strictEqual(diags.length, 0,
        `modified manifest still valid, got: ${diagMessages(diags)}`);
    } finally {
  ...
  test("custom editor handles invalid content gracefully", async () => {
    ...
    try {
      ...
      assert.ok(diags.length > 0,
        `invalid JSON produces diagnostics in custom editor, got: ${diagMessages(diags)}`);
      ...
      await editor.edit((eb) => { eb.replace(fullRange, validContent); });

      const fixedDiags = await waitForDiagnostics(uri);
      assert.strictEqual(fixedDiags.length, 0,
        `fixed content clears diagnostics, got: ${diagMessages(fixedDiags)}`);
    } finally {
      await closeAllEditors();
      await deleteTempFile(uri);
    }
  ...
  test("custom editor coexists with diagnostics and tree view simultaneously", async () => {
    const content = JSON.stringify({
      ...
      components: [
        { id: "c1", kind: "cli", language: "rust", binaryName: "c1",
          expectedVersion: "1.0.0", platforms: ["linux-x64"], sources: ["path"] },
        { id: "c2", kind: "lsp", language: "rust", binaryName: "c2",
          expectedVersion: "1.0.0", platforms: ["linux-x64"], sources: ["bundled"] },
      ],
      hosts: {
        vscode: {
    ...
    try {
      ...
      const diags = vscode.languages.getDiagnostics(uri);
      assert.strictEqual(diags.length, 0,
        `coexist manifest clean, got: ${diagMessages(diags)}`);
      ...
      await editor.edit((eb) => { eb.replace(fullRange, broken); });

      const updatedDiags = await waitForDiagnostics(uri);
      assert.ok(updatedDiags.some((d) => d.message.includes("c2")),
      ...
      assert.ok(reDiags.some((d) => d.message.includes("c2")),
  ...
  test("opening multiple manifests in custom editors works independently", async () => {
    const contentA = JSON.stringify({
      ...
      components: [{
        id: "ma", kind: "cli", language: "rust", binaryName: "ma",
        expectedVersion: "1.0.0", platforms: ["linux-x64"], sources: ["path"],
      }],
    }, null, 2);
    ...
    try {
      ...
      assert.strictEqual(diagsA.length, 0,
        `multi-a is valid, got: ${diagMessages(diagsA)}`);
      assert.ok(diagsB.length >= 3,
        `multi-b has errors, got: ${diagMessages(diagsB)}`);
