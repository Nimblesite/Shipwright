...
suite("Extension Activation", () => {
  test("activates in a workspace with shipwright.json and registers all commands and providers", async () => {
    ...
    assert.ok(ours.length >= expected.length,
      `at least ${expected.length} shipwright commands, got ${ours.length}`);

    const folders = vscode.workspace.workspaceFolders;
    assert.ok(folders && folders.length > 0, "workspace has folders");

    const manifests = await vscode.workspace.findFiles(
      "**/shipwright.json", "**/node_modules/**", 5,
    );
  ...
  test("does not register duplicate commands on repeated activation calls", async () => {
    ...
    const before = (await vscode.commands.getCommands(true))
      .filter((c) => c.startsWith("shipwright."));
    ...
    assert.strictEqual(before.length, after.length,
      "command count unchanged after second activate");
