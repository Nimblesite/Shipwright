...
import {
  activateExtension, openDocument, setContent,
  waitForDiagnostics, closeAllEditors, createTempFile,
  deleteTempFile, manifest, diagMessages, openAndWaitForDiagnostics,
} from "./helpers";
...
suite("Manifest Validation Diagnostics", () => {
  ...
  test("full lifecycle: open valid manifest, corrupt it, fix field-by-field, close", async () => {
    ...
    assert.strictEqual(initial.length, 0,
      `valid manifest: expected 0 diags, got ${diagMessages(initial)}`);
    ...
    diags = await waitForDiagnostics(uri);
    assert.ok(diags.some((d) => d.message.includes("Product id")),
    ...
    diags = await waitForDiagnostics(uri);
    assert.ok(diags.some((d) => d.message.includes("Product version")),
    ...
    const valid = JSON.stringify({
      ...
      components: [{
        id: "r-lsp", kind: "lsp", language: "rust",
        binaryName: "r-lsp", expectedVersion: "2.0.0",
        platforms: ["linux-x64"], sources: ["bundled"],
      }],
    }, null, 2);
    await setContent(doc, valid);
    diags = await waitForDiagnostics(uri);
    assert.strictEqual(diags.length, 0,
      `restored valid manifest: expected 0 diags, got ${diagMessages(diags)}`);
  ...
  test("reports all missing fields for executable components and clears them one by one", async () => {
    ...
    try {
      ...
      assert.ok(diags.length >= 3,
        `expected >=3 diags for naked LSP, got ${diags.length}: ${diagMessages(diags)}`);
      assert.ok(diags.some((d) => d.message.includes("binaryName")), "missing binaryName");
      assert.ok(diags.some((d) => d.message.includes("expectedVersion")), "missing expectedVersion");
      assert.ok(diags.some((d) => d.message.includes("source")), "missing sources");
      ...
      const withBin = manifest({
        components: [{
          id: "naked-lsp", kind: "lsp", language: "rust",
          binaryName: "naked-lsp", platforms: ["linux-x64"],
        }],
      });
      await setContent(doc, withBin);
      diags = await waitForDiagnostics(uri);
      assert.ok(!diags.some((d) => d.message.includes("binaryName")), "binaryName error gone");
      assert.ok(diags.some((d) => d.message.includes("expectedVersion")), "expectedVersion still there");
      assert.ok(diags.some((d) => d.message.includes("source")), "sources still there");

      // add expectedVersion
      const withVer = manifest({
        components: [{
          id: "naked-lsp", kind: "lsp", language: "rust",
          binaryName: "naked-lsp", expectedVersion: "1.0.0", platforms: ["linux-x64"],
        }],
      });
      ...
      const complete = manifest({
        components: [{
          id: "naked-lsp", kind: "lsp", language: "rust",
          binaryName: "naked-lsp", expectedVersion: "1.0.0",
          platforms: ["linux-x64"], sources: ["bundled"],
        }],
      });
      ...
      diags = await waitForDiagnostics(uri);
      assert.strictEqual(diags.length, 0,
        `fully-fixed LSP: expected 0 diags, got ${diagMessages(diags)}`);
    } finally {
      await closeAllEditors();
  ...
  test("detects duplicate component ids, clears on dedup", async () => {
    const duped = manifest({
      components: [
        { id: "dup", kind: "cli", language: "rust", binaryName: "dup",
          expectedVersion: "1.0.0", platforms: ["linux-x64"], sources: ["path"] },
        { id: "dup", kind: "lsp", language: "rust", binaryName: "dup-lsp",
    ...
    try {
      ...
      assert.ok(diags.some((d) => d.message.includes("Duplicate component id")),
      ...
      const fixed = manifest({
        components: [
          { id: "comp-a", kind: "cli", language: "rust", binaryName: "comp-a",
            expectedVersion: "1.0.0", platforms: ["linux-x64"], sources: ["path"] },
          { id: "comp-b", kind: "lsp", language: "rust", binaryName: "comp-b",
            expectedVersion: "1.0.0", platforms: ["linux-x64"], sources: ["bundled"] },
        ],
      });
      ...
      assert.strictEqual(diags.length, 0,
        `deduped and complete: 0 diags, got: ${diagMessages(diags)}`);
    } finally {
      await closeAllEditors();
      await deleteTempFile(uri);
    }
  ...
  test("validates host activationVerifies references against actual component ids", async () => {
    const badRef = manifest({
      components: [
        { id: "real-lsp", kind: "lsp", language: "rust", binaryName: "real-lsp",
          expectedVersion: "1.0.0", platforms: ["linux-x64"], sources: ["bundled"] },
      ],
      hosts: {
        vscode: {
    ...
    try {
      ...
      assert.ok(diags.some((d) => d.message.includes("ghost-component")),
        `expected unknown component ref, got: ${diagMessages(diags)}`);
      assert.ok(diags.some((d) => d.message.includes("unknown component")),
      ...
      const fixed = manifest({
        components: [
          { id: "real-lsp", kind: "lsp", language: "rust", binaryName: "real-lsp",
            expectedVersion: "1.0.0", platforms: ["linux-x64"], sources: ["bundled"] },
          { id: "ghost-component", kind: "mcp", language: "rust", binaryName: "ghost",
            expectedVersion: "1.0.0", platforms: ["linux-x64"], sources: ["path"] },
        ],
        hosts: {
          vscode: {
      ...
      const twoBad = manifest({
        components: [
          { id: "only-real", kind: "cli", language: "rust", binaryName: "only",
            expectedVersion: "1.0.0", platforms: ["linux-x64"], sources: ["path"] },
        ],
        hosts: {
      ...
      assert.ok(refErrors.length >= 3,
        `expected >=3 ref errors, got ${refErrors.length}: ${diagMessages(refErrors)}`);
  ...
  test("warns on unknown platform identifiers and clears on fix", async () => {
    const badPlatform = manifest({
      components: [{
        id: "plat-test", kind: "cli", language: "rust", binaryName: "plat",
        expectedVersion: "1.0.0", platforms: ["linux-x64", "commodore-64"],
        sources: ["path"],
      }],
    });
    ...
    try {
      ...
      assert.ok(platWarns.length > 0,
        `expected platform warning, got: ${diagMessages(diags)}`);
      assert.strictEqual(platWarns[0].severity, vscode.DiagnosticSeverity.Warning,
      ...
      const fixedPlat = manifest({
        components: [{
          id: "plat-test", kind: "cli", language: "rust", binaryName: "plat",
          expectedVersion: "1.0.0", platforms: ["linux-x64", "darwin-arm64"],
          sources: ["path"],
        }],
      });
      await setContent(doc, fixedPlat);
      diags = await waitForDiagnostics(uri);
      assert.ok(!diags.some((d) => d.message.includes("Unknown platform")),
  ...
  test("warns when bundled config present but 'bundled' missing from sources", async () => {
    const mismatch = manifest({
      components: [{
        id: "bundle-test", kind: "lsp", language: "rust",
        binaryName: "bundle-test", expectedVersion: "1.0.0",
        platforms: ["linux-x64"],
    ...
    try {
      ...
      const bundleWarn = diags.filter((d) =>
        d.message.includes("bundled") && d.message.includes("missing from sources"));
      assert.ok(bundleWarn.length > 0,
        `expected bundled-source mismatch warning, got: ${diagMessages(diags)}`);
      ...
      diags = await waitForDiagnostics(uri);
      assert.strictEqual(diags.length, 0,
        `bundled warning cleared, got: ${diagMessages(diags)}`);
    } finally {
      await closeAllEditors();
      await deleteTempFile(uri);
    }
  ...
  test("validates all component kinds: executables need fields, extensions and assets do not", async () => {
    ...
    for (const kind of execKinds) {
      ...
      try {
        ...
        assert.ok(diags.some((d) => d.message.includes("binaryName")),
          `${kind}: requires binaryName`);
        assert.ok(diags.some((d) => d.message.includes("expectedVersion")),
          `${kind}: requires expectedVersion`);
        assert.ok(diags.some((d) => d.message.includes("source")),
    ...
    for (const kind of nonExecKinds) {
      ...
      try {
        ...
        const diags = await waitForDiagnostics(uri);
        assert.ok(!diags.some((d) => d.message.includes("binaryName")),
          `${kind}: no binaryName requirement`);
        assert.ok(!diags.some((d) => d.message.includes("expectedVersion")),
          `${kind}: no expectedVersion requirement`);
      } finally {
        await closeAllEditors();
        await deleteTempFile(uri);
      }
  ...
  test("asset component without asset block produces error", async () => {
    ...
    try {
      ...
      assert.ok(diags.some((d) => d.message.includes("asset") && d.message.includes("requires")),
      ...
      assert.strictEqual(fixed.length, 0,
        `asset with block: 0 diags, got: ${diagMessages(fixed)}`);
    } finally {
      await closeAllEditors();
      await deleteTempFile(uri);
    }
  ...
  test("multiple manifests open simultaneously get independent diagnostics", async () => {
    const good = manifest({
      components: [{
        id: "g", kind: "cli", language: "rust", binaryName: "g",
        expectedVersion: "1.0.0", platforms: ["linux-x64"], sources: ["path"],
      }],
    });
    const bad = manifest({
    ...
    try {
      ...
      assert.strictEqual(goodDiags.length, 0,
        `good manifest clean, got: ${diagMessages(goodDiags)}`);
      assert.ok(badDiags.length >= 3,
        `bad manifest has errors, got: ${diagMessages(badDiags)}`);
      ...
      assert.strictEqual(vscode.languages.getDiagnostics(uriBad).length, 0,
        `fixed bad manifest: 0 diags`);
    } finally {
      await closeAllEditors();
