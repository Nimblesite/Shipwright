...
suite("Extension Commands", () => {
  ...
  test("shipwright.validate scans workspace, runs validation on all manifests, reports results", async () => {
    ...
    try {
      ...
      assert.strictEqual(mainDiags.length, 0, `main manifest clean after validate, got: ${diagMessages(mainDiags)}`);

      const cmdDiags = vscode.languages.getDiagnostics(uri);
      assert.strictEqual(cmdDiags.length, 0, `temp manifest clean after validate, got: ${diagMessages(cmdDiags)}`);
    } finally {
      await closeAllEditors();
  ...
  test("shipwright.validate finds and reports issues across multiple shipwright.json files", async () => {
    ...
    try {
      ...
      assert.ok(badDiags.length >= 3, `broken manifest flagged, got ${badDiags.length}: ${diagMessages(badDiags)}`);
      ...
      const mainDiags = vscode.languages.getDiagnostics(vscode.Uri.joinPath(folder.uri, "shipwright.json"));
      assert.strictEqual(mainDiags.length, 0, `main manifest still clean, got: ${diagMessages(mainDiags)}`);
    } finally {
      await closeAllEditors();
  ...
  test("shipwright.refreshTree succeeds repeatedly without error", async () => {
    ...
    assert.ok(commands.includes("shipwright.refreshTree"), "command still registered after 5 invocations");
  });
  ...
  test("shipwright.platformMatrix opens a webview panel for the workspace manifest", async () => {
    ...
    assert.ok(tabsAfter > tabsBefore, `platformMatrix should open a new tab (before=${tabsBefore} after=${tabsAfter})`);

    const tabs = vscode.window.tabGroups.all.flatMap((g) => g.tabs);
    const matrixTab = tabs.find((t) => t.label.includes("Platform Matrix") || t.label.includes("test-product"));
  ...
  test("shipwright.openVisualEditor opens the custom editor for a manifest file", async () => {
    ...
    assert.ok(tabsAfter >= tabsBefore, "openVisualEditor opens or reuses a tab");
  });
  ...
  test("editing a manifest through a text editor triggers real-time revalidation", async () => {
    ...
    try {
      ...
      diags = await waitForDiagnostics(uri);
      assert.strictEqual(diags.length, 0, `undo restores clean state, got: ${diagMessages(diags)}`);
      ...
      await editor.edit((eb) => {
        eb.replace(fullRange, JSON.stringify(parsed, null, 2));
      });
      diags = await waitForDiagnostics(uri);
      assert.ok(
