...
fn stamp_cargo(root: &Path, version: &str, dry_run: bool, mut s: Summary) -> Result<Summary, StampError> {
    ...
    let replaced = replace_first_version_line(&original, version);
...
/// Rewrite the first `version = "..."` line found under any `[workspace.package]`
/// or `[package]` table. Deliberately narrow — does not parse TOML fully.
fn replace_first_version_line(src: &str, version: &str) -> String {
