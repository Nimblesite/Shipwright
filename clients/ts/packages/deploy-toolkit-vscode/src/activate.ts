...
export interface ActivateDeploymentToolkitOptions {
...
export async function activateDeploymentToolkit(
  context: ExtensionContextLike,
  options: ActivateDeploymentToolkitOptions = {}
): Promise<ActivationResult> {
  const extensionRoot = extensionRootPath(context);
  const manifestPath = options.manifestPath ?? path.join(extensionRoot, "deployment-toolkit.json");
  const manifest = await loadDeploymentManifest(manifestPath, options.readText);
...
async function probeCandidates(
  input: ResolveInput,
  options: Pick<ActivateDeploymentToolkitOptions, "env" | "execFile" | "timeoutMs">
